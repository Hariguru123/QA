...
#********************************************************************************************************************\
# 
#   File Name  :   Dhanush_buildscript_Arjuna.pl
#    
...
if($num_args < 4)
{ 
	print "Usage: perl Dhanush_buildscript_Arjuna.pl <1-7> <Daily/Release> <Trunk/Branch Repository name> <local path(Folder name where trunk or release codes will check out)>\n";
	...
	if(-d "$dhanushaon_path")
	{
		...
		else
		{
			...
			else
			{
				if(($subject eq "Weekly") || ($subject eq "Release"))
				{
					$subject1 = "Asthra Agni AONSensor Build - $sub_rel";
				}
				elsif($subject eq "Daily")
				{
					$subject1 = "Asthra Agni AONSensor Build";
				}
