...
print "Enter 4 for Android Build\n";
print "Enter 5 to build all subsystems\n";
...
{
	print "Please enter a value in between (1-5)\n";	
...
sub build_wearable()
{
	if(-d "$wearable_path")
	{
		...
		$status = system(". ./native_env.sh > log.txt; make clobber > clobber_log.txt 2> faillog.txt; make rel > buildlog.txt 2>> faillog.txt;");
		if ($status)
		{
		...
		{
			fcopy("$wearable_path/output/adv1image1/d_dhanush_wearable.bin", "$wearable_path/output/native1.bin");
			fcopy("$wearable_path/sdk/bin/native2.bin", "$wearable_path/output/native2.bin");
			...
			if(-f "$local_path/images.tar.gz")
			{
				...
				fcopy("$wearable_path/output/native1.bin", "$local_path/images/boot");
				fcopy("$wearable_path/output/native2.bin", "$local_path/images/boot");
				system("rm -rf $local_path/images.tar.gz");
			...
			fcopy("$wearable_path/output/native1.bin", "$android_path/images/boot");
			fcopy("$wearable_path/output/native2.bin", "$android_path/images/boot");
