...
$dir = "MSDK3_package_$date[1]_$date[2]";
$path = `pwd`;
...
$Scripts_path="http://192.168.24.194:9090/svn/swdepot/Dhanush/QA/scripts/mSDK-r_build_script";


#***************Creating a Directory for MSDK1 package****************************************************
chdir($dest_path);

print "\nCreating a directory for the mSDK-R package\n";
...
if($input eq 1)
{
	...
	system("sh package.sh MSDK3 >> packageshlog.txt");
	
	print "\n--->Deleting package.sh and readme.txt files from the AON folder\n";
	unlink("package.sh");
	unlink("readme.txt");
	
	print"\n--->Deleting 128 *128 and 320 * 240 folders\n";
...
if($input eq 1)
{
	...
	print "Native source packaging is not yet done,please choose another value\n";
	goto LOOPN; 
...
elsif($input eq 2)
{
	...
	system("sh package.sh MSDK3 >> Nativepackageshlog.txt");
	unlink("package.sh");
...
#*********************Packaging structure***********************
system("mkdir -p $rootpath/DHANUSH_MICRO_mSDK-R/Bootloader");
system("mkdir -p $rootpath/DHANUSH_MICRO_mSDK-R/Scripts");

print"\n***************Renaming the directories***********\n";
chdir($rootpath);
system("svn co $Scripts_path >> $dest_path/logs/Scripts_checkout_log");

system("cp -r AON DHANUSH_MICRO_mSDK-R/");
system("cp -r Native DHANUSH_MICRO_mSDK-R/");
system("cp $dest_path/Pre-binaries/spiDump.bin DHANUSH_MICRO_mSDK-R/Bootloader");
system("cp $dest_path/Pre-binaries/spiutil.elf DHANUSH_MICRO_mSDK-R/Bootloader");
system("cp $dest_path/Pre-binaries/dictionary.bin DHANUSH_MICRO_mSDK-R/Native/");
system("cp mSDK-r_build_script/build_script_MSDK3.pl DHANUSH_MICRO_mSDK-R/Scripts/Build_Script.pl");

chdir("$rootpath/DHANUSH_MICRO_mSDK-R");
...
system("cp -r $toolchain_mips $rootpath/DHANUSH_MICRO_mSDK-R/Tools/toolchain/ubuntu64");
...
print"\n--->Creating a docs folder\n";
mkdir("Docs");
chdir("$rootpath/DHANUSH_MICRO_mSDK-R/Docs");
...
chdir("$rootpath/DHANUSH_MICRO_mSDK-R/Scripts");
